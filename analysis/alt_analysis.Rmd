...
The actual model consists of a nimble code block that describes the relationships and sets the priors for the parameters. It should be noted that we have changed the function for the number of settlements, and thus the proposal distribution. It is now a Negative Binomial Distribution, where both the expected value and the variance can vary. The expected value is still derived from $\lambda_t$, but this is integrated into the model via a success parameter $p_t$, which results as follows:
...
model_code <- nimbleCode( {
  ...
  nEnd ~ dnorm(z[nYears] * MeanSiteSize / AreaSwissPlateau, sd=0.5)
...
```{r}
model_monitors <- c("PopDens", "beta", "r")
```
...
```{r}
ggsave(normalizePath(file.path(here(), "figures","beta_estimation_neg_binomial.pdf"), mustWork = F),

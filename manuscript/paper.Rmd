---
title: "Bayesian Hierarchical Model for estimating holocene prehistoric population development on the Swiss Plateau"
author:
  - Martin Hinz:
...
my_basemap <- ggmap(swiss_background, darken = c(0.6, "white"))
...
<!-- JR: Might the following four paragraphs fit better in a background section ("Prehistoric demography")? -->
...
<!-- JR: Begin by defining, broadly, what Bayesian statistics is? -->
...
<!-- JR: The above is interesting, but maybe a bit general for this paper? It's also conspicuously lacking in citations for a background section. I'd suggest condensing and moving it to the introduction. -->
...
<!-- JR: Non-Europeanists (like me!) might appreciate a brief summary of the geography and prehistory of the Swiss plateau here. -->
...
<!-- JR: The rest of this section is a bit and I lost the thread a bit. Could we first have a brief summary of which proxies we will use, and why (helping to explain why the Swiss Plateau was selected), then further details on each proxy in labelled sub-sections? -->
...
<!-- JR: Would be nice to add rivers and lakes to the base map. -->
...
```{r compdendroc14, fig.cap="Comparison of the scaled number of ^14^C and dendrodated sites over time in the dataset used."}
joined_data <- read.csv(file.path(rstudioapi::getActiveProject(), "data", "preprocessed_data", "all_proxies.csv"))

ggplot(joined_data) + geom_line(aes(x=1950-age, y=scale(sumcal, center = F), color = "darkred"), alpha = 0.75)  + geom_line(aes(x=1950-age, y=scale(dendro, center = F), color = "darkblue"), alpha = 0.75) + theme_minimal()  +
    labs(x = "years BCE",
         y = "scaled values",
         color = "Legend") + 
...
<!-- JR: I don't understand what "scaled values" means here. -->
...
<!-- JR: At the resolution the figure is rendered at, I don't think the 1 km jitter is perceptible. -->
...
ggplot(joined_data) + geom_line(aes(x=1950-age, y=aoristic_sum, color = "darkgray"), alpha = 0.75) +
  theme_minimal()  +
    labs(x = "years BCE",
         y = "aoristic sum",
         color = "Legend") + 
...
knitr::include_graphics(file.path(rstudioapi::getActiveProject(), "analysis", "analysis_files", "figure-html", "pollenproxy-1.png"))
```

Unfortunately, in the Swiss Plateau there is no data from burials that could be usefully incorporated into such this model. Preservation conditions again simply prevent their use in this study. Nevertheless, we see a very high potential for other regions in the integration of demographic indicators from burial data in order to enlarge the canon of methods and the range of proxies.

<!-- JR: What preservation conditions? There are prehistoric burials and cemeteries in Switzerland, no? -->
...
<!-- JR: The last bit repeats an earlier point. -->
...
The gamma distribution used centres the probability in a range $[0,1[$, adding 1 makes this range $[1-2[$ (See also Figure \@ref(fig:gammadist)).

<!-- JR: A whole figure just to illustrate the gamma distribution seems like overkill. Could we add the prior distributions of other parameters? Or just get rid of it: you can always look up the shapes on Wikipedia :) --> 

```{r gammadist, fig.cap="The gamma-distributed maximum growth rate prior of the model."}
...
ggplot(expert_estimations) + geom_segment(aes(x=from, xend = to, y = estimate, yend = estimate, color = source)) + xlab("cal BCE") + ylab("Density p/km²") + theme_minimal()
```

<!-- JR: I really like the idea of discussing "dead ends" in the model design process – very modern and reflexive! Maybe hightlight these in a section of their own? -->
...
<!-- JR: I don't know where it would fit best, but I'd find it very helpful to see a table of all the priors and fixed parameters with their initial values. -->

# Results

<!-- JR: I really think we need a section evaluating and validating the model fit before we discuss its output. -->

```{r popdensplot, fig.cap="The estimate of the population density resulting from the model and the three proxies, which are also plotted (scaled) for comparison."}
...
<!-- JR: I would split this into subsections evaluation a) the merits of the approach and b) the specific results for Switzerland. -->
